#!/usr/bin/env bash
# Rule-based validator for config/generator/plan/*/plan.yaml files. Rules live in
...
#
# Usage:
#   data-caterer/script/validate/validate-plan.sh                 # every tracked plan
#   data-caterer/script/validate/validate-plan.sh <plan-file>...   # specific plan(s)
set -euo pipefail
...
if [ "$#" -eq 0 ]; then
  set -- config/generator/plan/*/plan.yaml
fi
...
docker run --rm \
  ...
  -w /workspace \
  "$PYTHON_IMAGE" \
  python3 data-caterer/script/validate/validate_plan.py "${args[@]}"
